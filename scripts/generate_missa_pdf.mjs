...
export async function fetchMissa(divinumDate) {
  console.log(`[Divinum Officium] Fetching Mass for date: ${divinumDate}...`);
...
function cleanCell(rawHtml, isLatin = false) {
  if (!rawHtml) return '';
  ...
  // User requirement: "add the rubrics only in english"
  // For Latin (isLatin = true): keep speaker tags (S., M.) and versicles (℣., ℟.), but strip instructional rubrics and citations.
  // For English (isLatin = false): format instructional rubrics in red italics, citations, and speaker tags.
  c = c.replace(/<FONT[^>]*COLOR=["']?red["']?[^>]*><I>([\s\S]*?)<\/I><\/FONT>/gi, (match, p1) => {
  ...
  c = c.replace(/<FONT[^>]*COLOR=["']?red["']?[^>]*>([\s\S]*?)<\/FONT>/gi, (match, p1) => {
    if (isLatin) return '';
...
export function parseMissaHtml(html) {
  ...
  trMatches.forEach((tr, index) => {
    ...
    if (tds.length === 2) {
      const lClean = tds[0].replace(/<TD[^>]*>/gi, '').replace(/<\/TD>/gi, '').replace(/<DIV ALIGN=["']?right["']?>[\s\S]*?<\/DIV>/gi, '').trim();
      const eClean = tds[1].replace(/<TD[^>]*>/gi, '').replace(/<\/TD>/gi, '').replace(/<DIV ALIGN=["']?right["']?>[\s\S]*?<\/DIV>/gi, '').trim();
      ...
      if (lRaw.length === eRaw.length && lRaw.length > 1) {
        for (let i = 0; i < lRaw.length; i++) {
          const cleanL = cleanCell(lRaw[i], true);
          const cleanE = cleanCell(eRaw[i], false);
          if (cleanL || cleanE) {
        ...
        const cleanLatin = cleanCell(tds[0], true);
        const cleanVernacular = cleanCell(tds[1], false);
...
export function buildHtmlDocument({ feast, commemoration, rubricsVersion, items, dateStr }) {
  let bodyHtml = '';
  ...
  <style>
    ...
    * {
    ...
    .feast-container {
      ...
      border: 1.5px solid #C5A059; box-shadow: 0 0 0 3px rgba(197, 160, 89, 0.15);
      border-radius: 6px;
    ...
    /* Illuminated Orthodox-style Initial Capital (Буквица) */
    .prayer-initial {
      font-family: 'Cinzel Decorative', 'Cinzel', serif;
      font-size: 1.45em;
      line-height: 0.9;
...
async function main() {
  ...
  // Copy to conversation artifact directory for immediate viewing
  const artifactDir = '/Users/raenard/.gemini/antigravity/brain/f9a5d215-5a65-4157-a158-d3dcabf44f5b';
  if (fs.existsSync(artifactDir)) {
    const artifactPdfPath = path.join(artifactDir, outputFileName);
    fs.copyFileSync(rootPdfPath, artifactPdfPath);
    console.log(`[Artifact] Copied PDF to ${artifactPdfPath}`);
  }
